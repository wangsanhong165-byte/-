...
| 😊 | **会动的 Live2D 形象** | 说话时**口型同步**、情绪驱动**表情变化**（含口是心非的双情绪泄露弧）、自然微动与安全动作编排；身体第一通道（深躯干轴）、能量分档与三套动力学原型，待机按上一段对话的**情绪残留**自动染色 |
| 🧠 | **持久记忆** | 记得你是谁、聊过什么；SQLite + FTS5 + 时间语义 + 向量通道（本地小模型：嵌入召回 + 交叉编码器精排），带冲突仲裁与每周反思的后台编译管线，陪伴越久人格越完整 |
...
| 记忆 | SQLite + FTS5 + 向量通道（Qwen3-Embedding-0.6B 召回 / Qwen3-Reranker-0.6B 精排，进程内 CPU/GPU 小模型），后台编译与每周反思 |
