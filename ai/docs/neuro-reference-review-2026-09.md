...
>
> **📋 落地状态注记（2026-09-06）**：Gap 清单后续处置——#1 表情过渡 360ms 保留、#2/#6/#7/#8 已落地（blinkRate modifier 现有消费方 controllers.ts:807）、#3 振幅 +30% 撤回、#5 idle 间隔已被 6.5-13s 重设计取代。正文 §"与本项目的同构性"中 "`naturalVAD` 双通道" 的表述已过时：协议瘦身后 naturalVAD 不再是 LLM 输出字段，改为按 emotion 本地派生（character_intent.EMOTION_VAD）；双情绪改由 `leak` 字段 + LeakArcScheduler 表达。历史行号为当日检出值，现读需对照最新代码。
